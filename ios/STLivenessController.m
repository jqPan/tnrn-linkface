...
- (void)willResignActive {
    ...
    if (self.controllerDelegate &&
        ...
        [self.mainQueue addOperationWithBlock:^{
            [self.controllerDelegate livenessControllerDeveiceError:STID_WILL_RESIGN_ACTIVE];
        }];
    }
...
- (void)setupUI {
    ...
    for (int i = 0; i < _detectionArr.count; i++) {
        SZNumberLabel *lblStepNumber =
            [[SZNumberLabel alloc] initWithFrame:self.is3_5InchScreen ? CGRectMake(i * 20.0 + i * 4.0, 0, 16.0, 16.0) :
...
- (void)showPromptWithDetectionType:(LivefaceDetectionType)iType detectionIndex:(NSInteger)index {
    if (self.currentAudioPlayer) {
        [self stopAudioPlayer];
    }

    SZNumberLabel *lblNumber = [self.stepBackGroundView viewWithTag:kSTViewTagBase + index];
                            ...
                            if (granted) {
                                ...
                                if (self.controllerDelegate &&
                                    ...
                                    [self.mainQueue addOperationWithBlock:^{
                                        ...
                                        [self.controllerDelegate livenessControllerDeveiceError:STID_E_CAMERA];
                                    }];
                                }
                            }
                        }];
...
- (void)livenessTrackerDistanceStatus:(LivenesssTrackerFaceDistanceStatus)distanceStatus
                          boundStatus:(LivenesssTrackerFaceBoundStatus)boundStatus
                      isFaceOcclusion:(BOOL)isFaceOcclusion
                            faceModel:(STFace *)faceModel {
    ...
    if ((currentTime - self.lastUpdateTime) > 300) {
        ...
        } else if (distanceStatus == STID_FACE_TOO_FAR) {
            self.trackerPromptLabel.text = @"请移动手机靠近面部";
        } else if (distanceStatus == STID_FACE_TOO_CLOSE) {
            self.trackerPromptLabel.text = @"请移动手机远离面部";
        } else if (distanceStatus == STID_DISTANCE_FACE_NORMAL && boundStatus == STID_FACE_IN_BOUNDE) {
...
- (NSString *)faceOcclusionStringWithFaceModel:(STFace *)face {
    NSMutableString *tempStr = [[NSMutableString alloc] init];

    if (face.browOcclusionStatus == STID_OCCLUSION) {
        [tempStr appendFormat:@"眉毛、"];
    }
    if (face.eyeOcclusionStatus == STID_OCCLUSION) {
        [tempStr appendFormat:@"眼睛、"];
    }
    if (face.noseOcclusionStatus == STID_OCCLUSION) {
        [tempStr appendFormat:@"鼻子、"];
    }
    if (face.mouthOcclusionStatus == STID_OCCLUSION) {
...
- (void)livenessDidStartDetectionWithDetectionType:(LivefaceDetectionType)detectionType
...
- (void)livenessDidFailWithErrorType:(LivefaceErrorType)errorType
    ...
    if (self.delegate &&
        [self.delegate respondsToSelector:@selector(livenessDidFailWithErrorType:protobufData:requestId:images:)]) {
        [self.mainQueue addOperationWithBlock:^{
            [self.delegate livenessDidFailWithErrorType:errorType
                                           protobufData:protobufData
                                              requestId:requestId
                                                 images:imageArr];
        }];
    }
#pragma mark livenessDidFailWithErrorType
    [STStartAndStopIndicatorView sharedIndicatorStopAnimate];
#pragma mark livenessDidFailWithErrorType
}
